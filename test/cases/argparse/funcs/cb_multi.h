...
{
  {
    ...
    struct argparse_option options[] = {
        OPTION_STRING('s', "string", NULL, &string,
  ...
  }
  {
    ...
    struct argparse argparse;
    struct argparse_option options[] = {
        OPTION_STRING('s', "string1", NULL, &string1,
                      argparse_callback_multiple_arguments,
                      (intptr_t)&array_size1),
        OPTION_STRING('t', "string2", NULL, &string2,
                      argparse_callback_multiple_arguments,
                      (intptr_t)&array_size2),
        OPTION_END()};
    argparse_init(&argparse, options, NULL);
    argparse_parse(
        &argparse, 7,
        (char *[]){"-s", "hello", "world", "-t", "foo", "bar", "baz"});
  ...
  }
  {
    ...
    argparse_parse(&argparse, 6,
                   (char *[]){"--string1", "test1", "--string2", "test2",
                              "test3", "test4"});
  ...
  }
  {
    ...
    struct argparse_option options[] = {
        ...
        OPTION_END()};
    argparse_init(&argparse, options, NULL);
    argparse_parse(&argparse, 4,
                   (char *[]){"--string", "test", "-s", "another"});
    EXPECT_EQUAL_INT(array_size, 1);
    EXPECT_NOT_NULL(string);
  ...
  }
  {
    ...
    argparse_parse(
        &argparse, 8,
        (char *[]){"--string1", "a1", "-t", "b1", "b2", "-s", "a2", "a3"});
    EXPECT_EQUAL_INT(array_size1, 2);
