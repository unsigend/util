...
UTEST_TEST_CASE(single)
{
  {
    bool force = false;
    struct argparse argparse;
    struct argparse_option option_list[] = {
        OPTION_BOOLEAN('f', "force", NULL, &force, NULL, 0), OPTION_END()};
  ...
  struct argparse_option mixed_options[] = {
      ...
      OPTION_BOOLEAN('q', "quiet", NULL, &quiet, NULL, 0), OPTION_END()};
  ...
  verbose = false;
  quiet = false;
  argparse_parse(&argparse_mixed, 4,
                 (char *[]){"-v", "--quiet", "--verbose", "-q"});
  EXPECT_TRUE(verbose);
  EXPECT_TRUE(quiet);

  {
    ...
    struct argparse argparse;
    struct argparse_option option_list[] = {
        OPTION_INT('n', "number", NULL, &number, NULL, 0), OPTION_END()};
    argparse_init(&argparse, option_list, NULL);
  ...
  }
  {
    ...
    struct argparse_option fixed_options[] = {
        OPTION_INT('o', "one", NULL, &number, NULL, 0), OPTION_END()};
  ...
  }
  {
    ...
    struct argparse_option fixed_options[] = {
        ...
        OPTION_INT('t', "two", NULL, &number, NULL, 0), OPTION_END()};
    argparse_init(&argparse_fixed, fixed_options, NULL);
    argparse_parse(&argparse_fixed, 2, (char *[]){"-o", "10"});
    EXPECT_EQUAL_INT(number, 10);
    ...
    argparse_parse(&argparse_fixed, 6,
                   (char *[]){"-o", "30", "--one", "40", "-t", "50"});
    EXPECT_EQUAL_INT(number, 50);

    argparse_parse(
        &argparse_fixed, 4,
        (char *[]){"-o", "30", "--one", "40", "-t", "50", "--two", "60"});
    EXPECT_EQUAL_INT(number, 40);
...

  {
    ...
    struct argparse argparse;
    struct argparse_option option_list[] = {
        OPTION_DOUBLE('n', "number", NULL, &number, NULL, 0), OPTION_END()};
    argparse_init(&argparse, option_list, NULL);
...

  {
    ...
    struct argparse argparse;
    struct argparse_option option_list[] = {
        OPTION_STRING('s', "string", NULL, &string, NULL, 0), OPTION_END()};
    argparse_init(&argparse, option_list, NULL);
