...
{
  {
    ...
    struct argparse_option options[] = {
        ...
        OPTION_BOOLEAN('z', "flag3", NULL, &flag3, NULL, 0), OPTION_END()};
  ...
  }
  {
    ...
    struct argparse_option options[] = {
        ...
        OPTION_INT('c', "third", NULL, &num3, NULL, 0), OPTION_END()};
    argparse_init(&argparse, options, NULL);
    ...
    argparse_parse(
        &argparse, 6,
        (char *[]){"--first", "100", "--second", "200", "--third", "300"});
  ...
  }
  {
    ...
    struct argparse_option options[] = {
        ...
        OPTION_DOUBLE('f', "double3", NULL, &val3, NULL, 0), OPTION_END()};
    argparse_init(&argparse, options, NULL);

    argparse_parse(&argparse, 6,
                   (char *[]){"-d", "1.5", "-e", "2.5", "-f", "3.5"});
    EXPECT_EQUAL_DOUBLE(val1, 1.5);
    EXPECT_EQUAL_DOUBLE(val2, 2.5);
    EXPECT_EQUAL_DOUBLE(val3, 3.5);

    argparse_parse(&argparse, 6,
                   (char *[]){"--double1", "10.5", "--double2", "20.5",
                              "--double3", "30.5"});
  ...
  }
  {
    ...
    struct argparse_option options[] = {
        ...
        OPTION_STRING('v', "value", NULL, &str3, NULL, 0), OPTION_END()};
    argparse_init(&argparse, options, NULL);

    argparse_parse(&argparse, 6,
                   (char *[]){"-p", "/tmp", "-n", "test", "-v", "data"});
    EXPECT_EQUAL_STRING(str1, "/tmp");
    EXPECT_EQUAL_STRING(str2, "test");
    EXPECT_EQUAL_STRING(str3, "data");

    argparse_parse(&argparse, 6,
                   (char *[]){"--path", "/usr", "--name", "config", "--value",
                              "settings"});
